...
export class MasterServiceService {
  ...
  constructor(private http:HttpClient) {
   ...
   public listeOperation(dateDebut,dateFin): Promise<any>{
    let params="param="+JSON.stringify({dateDebut:dateDebut,dateFin:dateFin,token:this.token,timetampe:(new Date()).toString()});
    //console.log(params);
  ...
  public listeUser(): Promise<any>{
    let params="param="+JSON.stringify({token:this.token,timetampe:(new Date()).toString()});
    //console.log(params);
  ...
  public listedeposit(dateDebut,dateFin,id_user): Promise<any>{
    let params="param="+JSON.stringify({dateDebut:dateDebut,dateFin:dateFin,token:this.token,id_user:id_user,timetampe:(new Date()).toString()});
  ...
  public updateCaution(montant,id_user): Promise<any>{
    let params="param="+JSON.stringify({montant:montant,token:this.token,id_receiver:id_user,timetampe:(new Date()).toString()});
